...
function fileEntryStub(file, parentPath) {
  return {
    isFile: true,
    isDirectory: false,
    name: file.name,
...
function directoryEntryStub(directory, parentPath) {
  const fullPath = `${parentPath}/${directory.name}`;
  const children = [...(directory.files ?? []).map(file => fileEntryStub(file, fullPath)), ...(directory.directories ?? []).map(subdirectory => directoryEntryStub(subdirectory, fullPath))];
  return {
    isFile: false,
    isDirectory: true,
    name: directory.name,
...
/**
  ...
  @return {Promise}
 */
async function dragAndDropDirectory(target, options) {
  const dropzone = target instanceof HTMLElement ? target : find(target);
  assert(`Target '${dropzone}' could not be found.`, dropzone);
  assert('options.directories must be an array', options.directories instanceof Array);
  assert('All loose files must be instances of File type', (options.files ?? []).every(file => file instanceof File));
...
export { dragAndDrop, dragAndDropDirectory, dragEnter, dragLeave, selectFiles };
